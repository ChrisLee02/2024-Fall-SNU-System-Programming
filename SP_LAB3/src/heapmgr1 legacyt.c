...
static Chunk_T merge_chunk(Chunk_T c1, Chunk_T c2) {
  ...
  // todo: footer 정보를 업데이트하도록

  Chunk_T c3 = chunk_get_next_free_chunk(c2);
  chunk_set_units(c1, chunk_get_units(c1) + chunk_get_units(c2) + 2);
  chunk_set_next_free_chunk(c1, c3);
  if (c3 != NULL) {
    Chunk_F c3_footer =
        chunk_get_footer_from_header(c3, g_heap_start, g_heap_end);

    assert(c3_footer != NULL);
    chunk_footer_set_prev_free_chunk(c3_footer, c1);
  }
  return c1;
}
/*--------------------------------------------------------------------*/
...
static Chunk_T split_chunk(Chunk_T c, size_t units) {
  ...
  // chunk_set_next_free_chunk(c2, chunk_get_next_free_chunk(c)); # 이거 왜
  // 넣은거?

  chunk_set_next_free_chunk(c, c3);
  if (c3 != NULL) {
    Chunk_F c3_footer =
        chunk_get_footer_from_header(c3, g_heap_start, g_heap_end);

    assert(c3_footer != NULL);
    chunk_footer_set_prev_free_chunk(c3_footer, c);
  }
  return c2;
}
/*--------------------------------------------------------------------*/
...
static void insert_chunk(Chunk_T c) {
  ...
  if (g_free_head == NULL) {
    ...
    chunk_footer_set_prev_free_chunk(
        chunk_get_footer_from_header(c, g_heap_start, g_heap_end), NULL);
  } else {
    ...
    chunk_footer_set_prev_free_chunk(
        chunk_get_footer_from_header(c, g_heap_start, g_heap_end), NULL);
    chunk_footer_set_prev_free_chunk(
        chunk_get_footer_from_header(g_free_head, g_heap_start, g_heap_end), c);
