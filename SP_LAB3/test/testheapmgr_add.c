...
static void get_args(int argc, char *argv[], int *pi_test_num, int *pi_count,
                     int *pi_size);
...
static char *apc_test_name[] = {"LIFO_fixed",  "FIFO_fixed",   "LIFO_random",
                                "FIFO_random", "random_fixed", "random_random",
                                "worst"};
...
static test_function apf_test_function[] = {test_LIFO_fixed_malloc,
                                            test_FIFO_fixed_malloc,
                                            test_LIFO_random_malloc,
                                            test_FIFO_random_malloc,
                                            test_random_fixed,
                                            test_random_random,
                                            test_worst,
                                            test_LIFO_fixed_free,
                                            test_FIFO_fixed_free,
                                            test_LIFO_random_free,
                                            test_FIFO_random_free};
...

{
  ...
  if (i_test_num < 4) {
    ...
    d_malloc_time =
        ((double)(i_malloc_clock - i_initial_clock)) / CLOCKS_PER_SEC;
...

{
  ...
  if (i == i_test_count) {
    ...
    for (i = 0; i < i_test_count; i++) fprintf(stderr, " %s", apc_test_name[i]);
    fprintf(stderr, "\n");
    exit(EXIT_FAILURE);
  }
...

{
  ...
  s_rlimit.rlim_cur = 1000;
  s_rlimit.rlim_max = 1000;
...
{
  if (!i_successful) fprintf(stderr, "Test at line %d failed.\n", i_lineNum);
}
...

{
  ...
  for (i = 0; i < i_count; i++) {
...
#ifndef NDEBUG
    {
      ...
      int i_col;
      char c = (char)((i % 10) + '0');
      for (i_col = 0; i_col < i_size; i_col++) apc_chunks[i][i_col] = c;
    }
#endif
  }
}
...

{
  ...
  /* Fill ai_sizes, an array of random integers in the range 1 to
     i_size. */
  for (i = 0; i < i_count; i++) ai_sizes[i] = (rand() % i_size) + 1;
  ...
  for (i = 0; i < i_count; i++) {
...
#ifndef NDEBUG
    {
      ...
      int i_col;
      char c = (char)((i % 10) + '0');
      for (i_col = 0; i_col < ai_sizes[i]; i_col++) apc_chunks[i][i_col] = c;
    }
#endif
  }
}
...

{
  ...
  for (i = 0; i < i_count; i++) {
...
#ifndef NDEBUG
    {
      ...
      for (i_col = 0; i_col < i_size; i_col++) apc_chunks[i_rand][i_col] = c;
    }
#endif
...

{
  ...
  while (i < i_count) {
    apc_chunks[i] =
        heapmgr_malloc((size_t)(((size_t)i * i_size / i_count) + 1));
...
#ifndef NDEBUG
    {
      ...
      char c = (char)((i % 10) + '0');
      for (i_col = 0; i_col < max; i_col++) apc_chunks[i][i_col] = c;
    }
#endif
    i++;
  ...
  while (i >= 2) {
...
#ifndef NDEBUG
    {
      ...
      size_t max = ((size_t)i * i_size / i_count) + 1;
      char c = (char)((i % 10) + '0');
      for (i_col = 0; i_col < max; i_col++) ASSURE(apc_chunks[i][i_col] == c);
    }
#endif
    heapmgr_free(apc_chunks[i]);
  }
  ...
  /* Free all chunks. */
  for (i = 0; i < i_count; i++) heapmgr_free(apc_chunks[i]);
}